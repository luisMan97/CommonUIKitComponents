...
class ModalExampleViewController: UIViewController {
    ...
    private lazy var accordionView = AccordionView().then {
        $0.headerTitle = "Buttons configuration"
        $0.addContentView(alertButtonsOptionsStackView)
    }

    private let alertButtonsOptionsStackView = UIStackView().then {
        $0.spacing = 10
        $0.axis = .vertical
    }
    ...
    private var buttonsPadSpacing: Double = .zero
    ...
    private func addSubViews() {
        ...
        alertOptionsStackView.addArrangedSubview(accordionView)

        alertButtonsOptionsStackView.addArrangedSubview(buttonCenterVerticalToBottom)
        alertButtonsOptionsStackView.addArrangedSubview(buttonAligmntVertical)
        alertButtonsOptionsStackView.addArrangedSubview(underlineButtonsWhenHasNoBackgroundColorView)
        alertButtonsOptionsStackView.addArrangedSubview(secondaryButtonClearColor)
        alertButtonsOptionsStackView.addArrangedSubview(buttonsPadSpacingView)
        alertButtonsOptionsStackView.addArrangedSubview(primaryButtonCornerRadiusView)
        alertButtonsOptionsStackView.addArrangedSubview(secondaryButtonCornerRadiusView)
        alertButtonsOptionsStackView.addArrangedSubview(buttonPadCornerRadiusView)

        addConstraints()
    }

    private func addConstraints() {
        alertOptionsStackView.anchor(left: view.leftAnchor,
                                     paddingLeft: 8,
    ...
    private func showModal() {
        let modalConfiguration = ModalConfiguration()
            ...
            .setButtonPadAlignment(buttonAligment)
